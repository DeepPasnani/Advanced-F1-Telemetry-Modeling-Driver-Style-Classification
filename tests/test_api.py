...
class TestAPI:
    ...
    def test_load_and_analyze(self):
        # Load a session
        load_resp = client.post("/api/sessions/load", json={
            "year": 2023,
            "grand_prix": "Bahrain",
            "session_type": "R"
        })
        ...
        # List drivers
        drivers_resp = client.get(f"/api/sessions/{session_id}/drivers")
        assert drivers_resp.status_code == 200
        drivers = drivers_resp.json()["data"]
        assert len(drivers) > 0

        # Run analysis on first 3 drivers
        analyze_resp = client.post(f"/api/sessions/{session_id}/analyze", json={
        ...
        # Get report
        report_resp = client.get(f"/api/analysis/{analysis_id}/report")
        assert report_resp.status_code == 200
        assert len(report_resp.json()["data"]["report"]) > 0

    def test_invalid_session(self):
        resp = client.get("/api/sessions/nonexistent/drivers")
